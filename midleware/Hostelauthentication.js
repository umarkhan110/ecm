...
const Hostelauthenticate = async (req, res, next) => {
    try {
        const token = req.cookies.jwtoken;
        //console.log(token);
        // yahan pr jwt.verify method token ko verify kara secret key ka sath
        const verifyToken = jwt.verify(token, process.env.SECRET_KEY);
        //console.log( "Ya ha token baba" + verifyToken._id);
        const rootUser = await Hostelusers.findOne({ _id: verifyToken._id, "tokens.token": token });
...
module.exports = Hostelauthenticate;